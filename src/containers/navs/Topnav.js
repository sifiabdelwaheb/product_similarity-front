...
import { UncontrolledDropdown, DropdownItem, DropdownToggle, DropdownMenu,Navbar, Input,NavbarBrand } from 'reactstrap';
...
class TopNav extends Component {
	constructor(props) {
		...
		this.state = {
			...
			searchKeyword: '',
			lang:
				(localStorage.getItem('currentLanguage') && localStorage.getItem('currentLanguage').toUpperCase()) ||
				'EN'
		};
	}
		...
		return (
			...
			<div className={"container"}>
			  ...
			  <img src={require('../../assets/images/bot.png')} className={Classes.Logo} />
				...
				<div style={{ width: '110px', marginRight: '50px' }}>
								...
								{this.props.auth.response.logo !== '' ? (
									<div className=" rounded-circle  societylogo  ">
										<img
											...
											src={'https://api-mobilar.wereact.co/' + this.props.auth.response.logo}
											alt="logo"
										/>
									</div>
								) : (
...
const mapDispatchToProps = (dispatch) => {
	return {
		setContainerClassnames: (parm1, param2, param3) => dispatch(setContainerClassnames(parm1, param2, param3)),
		...
		updateLanguage: (data, id) => dispatch(allLanguageActions.updateLanguageRequest(data, id))
	};
};
